...
export default function PaymentModal({ intent, status, onClose }: Props) {
  ...
  const isAlipay = intent.provider === 'alipay_sandbox';
  return (
    <div className="payment-modal-backdrop" role="presentation">
      <section className="payment-modal" role="dialog" aria-modal="true" aria-labelledby="payment-title">
        ...
        <p className="payment-modal-kicker">{isAlipay ? '支付宝沙箱支付' : 'FLASH MALL 本地沙箱'}</p>
        ...
        <p className="payment-modal-note">
          {isAlipay ? '请使用支付宝沙箱钱包扫码，本次演示不会产生真实扣款。' : '重复确认会复用同一支付事件，不会重复扣减库存。'}
        </p>
      </section>
    </div>
  );
