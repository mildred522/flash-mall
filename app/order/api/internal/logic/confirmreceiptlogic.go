...
func (l *ConfirmReceiptLogic) ConfirmReceipt(req *types.ConfirmReceiptReq, userID int64) (*types.ConfirmReceiptResp, error) {
	...
	// Update status: shipped(3) -> completed(4)
	result, err := db.ExecContext(l.ctx,
		"UPDATE orders SET status = 4, completed_at = NOW() WHERE id = ? AND status = 3", req.OrderId,
	)
	if err != nil {
	...
	_, _ = db.ExecContext(l.ctx,
		"INSERT INTO order_status_log (order_id, from_status, to_status, operator_id, remark) VALUES (?, 3, 4, ?, 'buyer confirmed receipt')",
		req.OrderId, userID,
	)
