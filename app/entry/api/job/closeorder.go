...
func (j *CloseOrderJob) ack(orderId string) {
	if _, err := j.svcCtx.Redis.ZremCtx(j.ctx, OrderProcessingQueueKey, orderId); err != nil {
		j.Errorf("close order ack processing queue remove failed: order_id=%s err=%v", orderId, err)
	}
	if _, err := j.svcCtx.Redis.DelCtx(j.ctx, OrderRetryKeyPrefix+orderId); err != nil {
		j.Errorf("close order ack retry key delete failed: order_id=%s err=%v", orderId, err)
	}
}

func (j *CloseOrderJob) onFailure(orderId string, err error) {
	retryKey := OrderRetryKeyPrefix + orderId
	retryCount, incrErr := j.svcCtx.Redis.IncrbyCtx(j.ctx, retryKey, 1)
	if incrErr != nil {
		j.Errorf("close order retry increment failed: order_id=%s err=%v", orderId, incrErr)
		return
	}
	if expireErr := j.svcCtx.Redis.ExpireCtx(j.ctx, retryKey, 24*60*60); expireErr != nil {
		j.Errorf("close order retry expire failed: order_id=%s err=%v", orderId, expireErr)
	}

	if retryCount > maxRetries {
		// CHG 2026-01-31: 变更=超过重试上限进入死信队列; 之前=无死信隔离; 原因=避免无限重试。
		if _, dlqErr := j.svcCtx.Redis.ZaddCtx(j.ctx, OrderDLQKey, time.Now().Unix(), orderId); dlqErr != nil {
			j.Errorf("close order DLQ add failed: order_id=%s err=%v", orderId, dlqErr)
			return
		}
	...
	if _, retryErr := j.svcCtx.Redis.ZaddCtx(j.ctx, OrderDelayQueueKey, nextAt, orderId); retryErr != nil {
		j.Errorf("close order retry enqueue failed: order_id=%s err=%v", orderId, retryErr)
		return
	}
	j.ack(orderId)
}
...
func (j *CloseOrderJob) handleCloseOrder(orderId string) error {
	...
	if order.Status != orderstatus.PendingPayment && order.Status != orderstatus.Closed {
		j.Infof("order status=%d, skip: %s", order.Status, orderId)
		return nil
	}
	...
	if j.svcCtx.CatalogCache != nil {
		j.svcCtx.CatalogCache.Invalidate(j.ctx)
	}
