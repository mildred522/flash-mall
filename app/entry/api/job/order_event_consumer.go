	...
	for d := range deliveries {
		if handleErr := c.handleDelivery(d); handleErr != nil {
			...
			if err := d.Nack(false, true); err != nil {
				c.Errorf("order event nack failed: msg_id=%s err=%v", d.MessageId, err)
			}
			continue
		}
		if err := d.Ack(false); err != nil {
			c.Errorf("order event ack failed: msg_id=%s err=%v", d.MessageId, err)
		}
	}
...
func (c *OrderEventConsumer) consumerTag() string {
	...
	if tag == "" {
		return "entry-api-order-event-consumer"
	}
	return tag
}
