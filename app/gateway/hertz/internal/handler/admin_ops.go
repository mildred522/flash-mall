...
func scanGatewayReconciliationIssues(ctx context.Context, db *sql.DB) (int64, error) {
	statements := []struct {
		...
		{query: `INSERT INTO reconciliation_issue (issue_key, issue_type, order_id, payment_order_id, expected_amount_fen, actual_amount_fen, severity, status, detail)
SELECT CONCAT('payment_amount_mismatch:', o.id, ':', p.id), 'payment_amount_mismatch', o.id, p.id,
       COALESCE(s.payable_amount_fen,0), COALESCE(p.payable_amount_fen,0), 3, 0, 'payment amount differs from order snapshot'
FROM orders o
...
ON DUPLICATE KEY UPDATE expected_amount_fen=VALUES(expected_amount_fen), actual_amount_fen=VALUES(actual_amount_fen),
  severity=VALUES(severity), status=0, detail=VALUES(detail)`, args: []any{paymentstatus.Success}},
		{query: `INSERT INTO reconciliation_issue (issue_key, issue_type, order_id, payment_order_id, severity, status, detail)
...
ON DUPLICATE KEY UPDATE severity=VALUES(severity), status=0, detail=VALUES(detail)`, args: []any{paymentstatus.Success, orderstatus.PendingPayment}},
		{query: `INSERT INTO reconciliation_issue (issue_key, issue_type, order_id, payment_order_id, severity, status, detail)
...
ON DUPLICATE KEY UPDATE severity=VALUES(severity), status=0, detail=VALUES(detail)`, args: []any{
			orderstatus.Paid, orderstatus.Shipped, orderstatus.Completed, orderstatus.RefundRequested, orderstatus.Refunded, paymentstatus.Success,
		}},
	}
	var affected int64
	for _, statement := range statements {
		result, err := db.ExecContext(ctx, statement.query, statement.args...)
		if err != nil {
			return 0, err
		}
