...
for deploy in auth-api entry-api order-rpc product-rpc inventory-kitex hertz-gateway; do
  echo "[LOCAL PROFILE] scale deployment/$deploy to 1"
  kubectl -n "$namespace" scale "deployment/$deploy" --replicas=1
done

for hpa in auth-api-hpa entry-api-hpa order-rpc-hpa product-rpc-hpa inventory-kitex-hpa hertz-gateway-hpa; do
