...
Write-Host "Applying schema through MySQL pod: $MysqlPod"

# CHG 2026-02-24: 变更=用管道传 SQL; 之前=使用 < 重定向导致 PowerShell 报错; 原因=PS 不支持这种重定向语法。
Get-Content -Raw scripts/k8s/schema.sql | kubectl -n $Namespace exec -i $MysqlPod -- mysql -uroot -p$RootPassword
