...
1. `app/order/api/order.api` 与实际 `routes.go`、`types.go` 不同步，后续 goctl 生成或接口维护存在破坏风险。
2. `app/order/api/desc/order.sql` 与 `scripts/k8s/init-db.sql` 不同步，订单库真实结构和描述源分裂。
3. 支付、发货、收货、退款主要在 order-api 直接改库，和下单主链路的 RPC/SAGA/Outbox 成熟度不一致。
...
3. 将 order-api 生命周期接口改为调用 order-rpc，API 层只负责鉴权、参数解析和响应转换。
...
1. `app/order/api/order.api`
2. `app/order/api/internal/types/types.go`
3. `app/order/api/internal/handler/routes.go`
4. `app/order/api/desc/order.sql`
5. `scripts/k8s/init-db.sql`

原则：

1. `order.api` 必须描述当前实际对外接口。
...
2. `go test ./app/order/api/internal/handler -count=1`
3. `go test ./app/order/api/... ./app/order/rpc/... ./app/product/rpc/... -count=1`
...
3. 修改 order-api 生命周期 handler 调用 RPC。
