...
- **可观测与性能验证：** 接入Prometheus、Grafana和Jaeger观测HTTP、RPC、缓存、库存及Outbox，构建基准、负载、压力、稳定性与故障恢复套件；4 vCPU环境下公开读3000 RPS档实际约2999.93 QPS且全部成功，订单写链路定位到25–40 RPS容量拐点，40次同幂等键请求仅生成1笔订单，RabbitMQ恢复后Outbox排空且最终一致性检查全部通过。

## 使用边界

- “约2999.93 QPS”是4 vCPU、约8 GiB WSL Docker固定环境的最高已测公开读档位，尚未触达读链路上限，不代表公网生产容量。
